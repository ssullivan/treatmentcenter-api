  ...
  public void extract() throws IOException {
    ...
    try {
      ...
      if (bucketAndObjectKey.isPresent()) {
        LOGGER.info("Successfully downloaded SAMSHA locator to bucket {}", bucketAndObjectKey.get().get_2());
      ...
      stopwatch.stop();
      LOGGER.info("[extract] Finished after {} seconds ({} ms)", stopwatch.elapsed(TimeUnit.SECONDS), stopwatch.elapsed(TimeUnit.MILLISECONDS));
    }
  }
  ...
  public void transform() throws IOException {
    ...
    try {
      if (!this.locatorObjectKey.isPresent() || !this.locatorBucket.isPresent()) {
        throw new IOException("No data to transform [bucket or locator key was empty]! Did you run extract?");
      }
      ...
      stopwatch.stop();
      LOGGER.info("[transform] Finished after {} seconds ({} ms)", stopwatch.elapsed(TimeUnit.SECONDS), stopwatch.elapsed(TimeUnit.MILLISECONDS));
    }
  }
  ...
  public void load() throws IOException {
    ...
    try {
      if (!this.samshaLocatorData.isPresent()) {
        throw new IOException("No data to transform [no parsed locator data available]! Did you run extract, and transform?");
      }
      ...
      if (samshaLocatorData.get().isGood()) {
        ...
        LOGGER.warn("The data feed {} @ {}/{} has bad/invalid data!", feedId, this.locatorBucket.get(), this.locatorObjectKey.get());
      }
    }
    finally {
      stopwatch.stop();
      LOGGER.info("[load] Finished after {} seconds ({} ms)", stopwatch.elapsed(TimeUnit.SECONDS), stopwatch.elapsed(TimeUnit.MILLISECONDS));
    }
  }
}
