...
public class PgFacilityDao implements IFacilityDao {
  ...
  private static Condition ST_DWithin(final GeoPoint geoPoint, final long radius,
      final GeoUnit unit) {
  ...
  private Integer[] convertToInts(final Map<String, Integer> lookup,
  ...
  public void addFacility(String feedId, Facility facility) throws IOException {
    ...
    dsl.transaction(configuration -> {
      ...
      innerDsl.insertInto(Tables.LOCATION)
          ...
          .set(Tables.LOCATION.LAT,
              facility.getLocation() != null ? facility.getLocation().lat() : null)
          .set(Tables.LOCATION.LON,
              facility.getLocation() != null ? facility.getLocation().lon() : null)
      ...
      dsl.transaction(configuration -> {
        ...
        final List<LocationRecord> records = batch.stream().map(facility -> {
          try {
            ...
            final Integer[] serviceCodeIds = convertToInts(serviceLookup,
                facility.getServiceCodes());
            final Integer[] categoryCodeIds = convertToInts(categoryLookup,
  ...
  public List<Facility> fetchBatch(Collection<String> ids) {
    return this.dsl.select(Tables.LOCATION.JSON)
        .from(Tables.LOCATION)
        .where(
            Tables.LOCATION.ID.in(ids.stream().map(ShortUuid::decode).collect(Collectors.toSet())))
